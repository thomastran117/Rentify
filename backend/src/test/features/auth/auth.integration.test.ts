...
describe("Auth persistence integration", () => {
  ...
  it("returns distinct available username suggestions and validates the limit", async () => {
    const defaultResponse = await persistenceApp.app.request(
      `http://rent.test${buildApiPath("/auth/username/suggestions")}`,
    ...
    expect(defaultBody.data.suggestions).toHaveLength(3);
    expect(new Set(defaultBody.data.suggestions).size).toBe(3);

    for (const suggestion of defaultBody.data.suggestions) {
      expect(suggestion).toMatch(/^[a-z]+-[a-z]+-[0-9]{4}$/);
      await expect(
        persistenceApp.prisma.profile.findUnique({
          where: { username: suggestion },
        }),
      ).resolves.toBeNull();
    }

    const customResponse = await persistenceApp.app.request(
      `http://rent.test${buildApiPath("/auth/username/suggestions?limit=5")}`,
      { headers: { origin: ORIGIN } },
    );
    ...
    expect(customBody.data.suggestions).toHaveLength(5);

    const malformedResponse = await persistenceApp.app.request(
      `http://rent.test${buildApiPath("/auth/username/suggestions?limit=11")}`,
      { headers: { origin: ORIGIN } },
    );
  ...
  it("authenticates through the Microsoft OAuth provider", async () => {
    ...
    expect(persistedUser?.profile?.username).toMatch(
      /^[a-z]+-[a-z]+-[0-9]{4}$/,
    );
    expect(persistedUser?.profile?.username).not.toBe("microsoft-user");
  });
